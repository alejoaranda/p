            ...
            ${employees.map(employee => `
                <tr>
                    ...
                    ${days.map(date => {
                        ...
                        const finalBgColor = colorMap[shift?.color?.replace('bg-','')] || 'transparent';
                        const finalTextColor = colorMap[shift?.textColor?.replace('text-','')] || '#333';
    ...
    const legend = `
        <div class="legend-container">
            ...
            <div class="legend">
                ...
                ${offShifts.map(s => {
                    const bgColor = colorMap[s.color.replace('bg-', '')] || s.color;
                    const textColor = colorMap[s.textColor?.replace('text-', '')] || '#ffffff';
                    return `
                    <div class="legend-item">
